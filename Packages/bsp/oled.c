...
{	
	...
	HAL_SPI_Transmit_DMA(WHICH_SPI,(uint8_t *)data,1); //需根据实际情况修改 
...
//
//
void OLED_Printf(uint8_t x, uint8_t y, uint8_t size1, const char *fmt, ...)
{
  ...
  oled_buf[len] = '\0';
  OLED_ShowString(x, y, oled_buf, size1);
}
//OLED的初始化
void OLED_Init(void)
{
