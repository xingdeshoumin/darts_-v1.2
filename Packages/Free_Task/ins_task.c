...
void INS_Task(void)
{
    ...
    // ins update
    if ((count % 1) == 0 && count > 1000)
    {
        mpu_get_data(&MPU6500);

        INS.Accel[X] = MPU6500.Accel[X];
        INS.Accel[Y] = MPU6500.Accel[Y];
        INS.Accel[Z] = MPU6500.Accel[Z];
        INS.Gyro[X] = MPU6500.Gyro[X];
        INS.Gyro[Y] = MPU6500.Gyro[Y];
        INS.Gyro[Z] = MPU6500.Gyro[Z];
        ...
        for (uint8_t i = 0; i < 3; i++) // 同样过一个低通滤波
        {
            INS.MotionVelocity_n[i] += INS.MotionAccel_n[i] * dt;
        }

        // 位置更新
        // for (uint8_t i = 0; i < 3; i++) // 同样过一个低通滤波
        // {
        //     INS.MotionPosition_n[i] += INS.MotionVelocity_n[i] * dt;
        // }
...
{
    PID_Calculate(&TempCtrl, MPU6500.Temperature, RefTemp);

    TIM_Set_PWM(&htim3, TIM_CHANNEL_2, float_constrain(float_rounding(TempCtrl.Output), 0, UINT32_MAX));
}
